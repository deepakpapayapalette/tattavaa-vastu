...
const ServiceDetail = () => {
    const ServiceDetailData = useLocation();
                  ...
                  <div className="row">
                      <div className="col-12">
                          <div className=" medical-tab-buttons mb-4 flex flex-wrap gap-3">
                              {categories.map((cat) => (
                                  <button key={cat.key}
                                      className={`cutom-tab-style py-[8px] px-[15px] rounded-lg text-[14px] ${activeCategory === cat.key ? "active-tab-btn-style" : ""}`}
